...
The three main folders refer to the three different tasks simulated to test the planner on aplanr RR robot. 
1. **RR_vertical_grasping** : the robot has to grasp the object from above, in this case the interaction is only on the y-axis; 
2. **RR_horizzontal_grasping**: the robot has to grasp the object from the side, in this case the interaction is on the x-axis and y-axis; 
3. **RR_placement**: he robot has to grasp the object from the side and place on a shelf with partially unknown parameters. 
